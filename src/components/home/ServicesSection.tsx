...
const ServicesSection = () => {
  return (
    <section id="services" className="py-20 bg-gradient-to-br from-gray-900 to-black text-white relative overflow-hidden">
      ...
      {/* Floating Elements and Glowing Orbs */}
      <FloatingElements count={12} className="opacity-30" />
      <GlowingOrb 
        size={250} 
        color="#8B1538" 
        intensity={0.2}
        className="top-1/4 left-1/6 animate-float"
      />
      <GlowingOrb 
        size={180} 
        color="#ff6b6b" 
        intensity={0.15}
        className="bottom-1/3 right-1/6 animate-float"
        ...
        <div className="max-w-6xl mx-auto">
          <ParallaxContainer speed={0.1}>
            <div className="text-center mb-16">
              <h2 className="text-5xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent transform-gpu hover:scale-105 transition-transform duration-300">
                Unsere Leistungen
              </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              ...
              <Link key={index} to={service.link} className="group block">
                <Card3D intensity={1.2} className="h-full">
                  <div className="absolute -inset-0.5 bg-gradient-to-r from-[#8B1538] to-red-600 rounded-2xl blur opacity-30 group-hover:opacity-100 transition duration-500"></div>
                  <div className="relative bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 hover:bg-gray-800/70 transition-all duration-500 border border-gray-700/50 h-full transform-gpu group-hover:shadow-2xl group-hover:shadow-[#8B1538]/20">
                    <div className="relative h-48 mb-6 overflow-hidden rounded-xl">
                      ...
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent group-hover:from-[#8B1538]/30 transition-all duration-500"></div>
                      <div className="absolute top-4 left-4 text-[#8B1538] bg-white/10 backdrop-blur-sm rounded-full p-3 mb-6 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">
                        {service.icon}
                      </div>
                    </div>
